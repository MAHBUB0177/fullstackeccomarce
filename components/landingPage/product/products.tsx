...
const Products = () => {
  ...
  const getAllProduct = async (currentPageNumber: number, payload: any) => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsLoading(false);
      }, 1000); // Adding a small delay for better UX
    }
  };
  ...
  return (
    <div className='shadow-lg bg-primary rounded-md w-full h-auto border border-slate-100 mt-8 px-4'>
      ...
      <Link href={'/productPage'} >
              <button
                className={"w-auto mt-5 text-sm p-[6px] font-semibold border-2 hover:bg-red-200 border-red-500  text-secondary rounded-md hover:scale-105 duration-300"}
              >
                Show More
