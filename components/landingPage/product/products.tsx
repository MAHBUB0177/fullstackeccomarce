...
const Products = () => {
  ...
  const getAllProduct = async (currentPageNumber: number, payload: any) => {
    setIsLoading(true);
    try {
      const res = await GetSearchProduct(currentPageNumber, 17, payload);
              ...
              productList?.map((item, i) => (
                <React.Fragment key={i}>
                  <div  className=" mr-3 mb-3">
                    ...
                    <div key={i} className="w-full cursor-pointer   overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md" onClick={() => handleOnClick(item?._id)}>
